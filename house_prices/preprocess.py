...
def scale_data(scalar, data: pd.DataFrame,
               is_test: bool = False) -> pd.DataFrame:
    if not is_test:
        scalar.fit(data)
...
def data_preprocessing(data: pd.DataFrame,
    ...
    list_of_features = ["OverallQual", "GrLivArea", "GarageCars",
                        ...
                        "YearRemodAdd", "BsmtFinSF1", "Foundation",
                        "LotFrontage", "WoodDeckSF", "MasVnrArea",
                        "Fireplaces", "ExterQual", "BsmtQual",
                        "KitchenQual", "GarageFinish", "GarageType",
                        "HeatingQC"]

    # To remove Columns
    unwanted_columns = ["PoolQC", "MiscFeature",
                        "Alley", "Fence", "FireplaceQu"]

    data = drop_columns(data, to_remove_columns=unwanted_columns)
